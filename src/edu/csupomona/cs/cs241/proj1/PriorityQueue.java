...
/**
 * This is a bounded queue with priority dequeuing. The lower the priority the
 * higher the number.
 * 
...
public class PriorityQueue<E extends Serializable> implements Serializable {
    ...
    /**
     * This keeps track of the order in which the elements are added. it also
     * acts as a unique id passed to the wrapper class
     */
     ...
     * @param size
     *            The max number of elements that there can be of each priority
     * @param priorities
     *            The number of priorities to use in your queue
     */
    ...
    PriorityQueue(int size, int priorities) {
        ...
        // Makes a normal queue to hold elements in original order, regardless
        // of priority
        normalQueue = (MyQueue<QueueItemShell<E>>) new MyQueue<QueueItemShell<E>>(
                size);

        // Makes an array of queues
        for (int i = 0; i < array.length; i++) {
            array[i] = (MyQueue<QueueItemShell<E>>) new MyQueue<QueueItemShell<E>>(
                    size);
        }
        orderCounter = 0;
        size = 0;
    ...
    /**
     * This adds the item passed to the queue with a specific priority. Low is
     * high. if the priority passed is less than 1 or greater than the greatest
     * priorities, it is truncated to 1 and the highest priority respectively.
     * 
     ...
     * @param priority
     *            The priority to add the element at
     */
    public void enqueue(E item, int priority) {
        ...
        // Makes sure that the priority given is in bounds of set priority, if
        // not changes it
        if (priority > array.length) {
            priority = array.length;
        }
        if (priority < 1) {
            priority = 1;
        }
        ...
        QueueItemShell<E> wrapper = new QueueItemShell<E>(item, priority,
                orderCounter++);
    ...
    public void enqueue(E item) {
        ...
        // Wraps the item to be added in a wrapper class and
        // stores data about the items position in the wrapper class
        QueueItemShell<E> wrapper = new QueueItemShell<E>(item, array.length,
                orderCounter++);
     ...
     * First it returns item by priority, if two elements are of the same
     * priority the one that was added first is removed first (FIFO)
     * 
    ...
    public E dequeueByPriority() {
        ...
        // Here it takes the item to be dequeued and returned and dumps out
        // element into a
        // temporary stack until it finds a match then also dequeues the match
        // from
    ...
    /**
     * Access the number of element in the queue.
     * 
     * @pre
     * @post
