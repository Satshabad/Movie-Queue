...
public class MovieQueue {
    /**
     * The queue of movies waiting to be sent to user.
     */
    private PriorityQueue<Movie> waitingQueue;

    /**
     * the queue of movies the user has at home.
     */
     ...
     *             when some internal error involving getting the persistent
     *             objects happens
     * @throws IOException
     *             when some internal error involving getting the persistent
     *             objects happens
     ...
     * 
     * @pre the atHomequeue should not be empty
     * @post the movie will be taken off of the home queue
     * 
     * @return the movie dequeued
     */
    public final Movie returnMovie() {
    ...
    public Movie[] displayMoviesAtHome() {
        ...
        // the code below is suppose to be how my program works but it doesnt.
        // ran out of time
        // MyQueue<Movie> cloneOfP = (MyQueue<Movie>)
        // DeepCopier.copy(atHomeQueue);
        Movie[] movieList = new Movie[cloneOfP.getSize()];
        int i = 0;
        while (!cloneOfP.isEmpty()) {
            movieList[i] = cloneOfP.dequeue();
